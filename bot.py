...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    data = query.data
    if data.startswith("page:"): await list_files(update, context, page=int(data[5:]))
    elif data.startswith("view:"): await show_detail(update, context, data[5:])
    elif data.startswith("del:"): await delete_file(update, context, data[4:])
...
async def show_detail(update: Update, context: ContextTypes.DEFAULT_TYPE, sid):
    ...
    # 恢复原始直连链接
    long_url = f"{SUPABASE_URL}/storage/v1/object/public/{SUPABASE_BUCKET_NAME}/{path}"
    ...
    await update.effective_chat.send_photo(photo=buf, caption=text, reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
